...
void gf256trimat_eval_m4f_112_44_publicinputs(uint8_t * y, const uint8_t * trimat, const uint8_t * x); // ov_publicmap in m4
...
void benchmark_gf256ov_publicmap(){
    ...
    uint8_t P[M * N * (N + 1) / 2];
    uint8_t sig[N];
    uint8_t acc[M], acc_mve[M], acc_m4[M];
    ...
    for (int l = 1; l <= TEST_RUN; l++) {
        ...
        sum_mve += cycles;
        bench_cycles3(gf256trimat_eval_m4f_112_44_publicinputs(acc_m4, P, sig), cycles);
        sum_m4 += cycles;
